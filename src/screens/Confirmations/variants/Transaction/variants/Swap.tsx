    ...
    <ConfirmationContent isFullHeight isTransaction transaction={transaction}>
      ...
      {isDevMode && data && (
        <>
          ...
          <Typography.Text style={{ color: theme.colorTextTertiary }}>{`simple id: ${data.id}`}</Typography.Text>
          <Typography.Text
            style={{ color: theme.colorTextTertiary }}>{`chainflip id: ${data.depositChannelId}`}</Typography.Text>
        </>
      )}
    </ConfirmationContent>
  );
