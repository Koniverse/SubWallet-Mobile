...
const filterAccount = (
  ...
  return (account: AccountProxy): boolean => {
    ...
    if (account.accounts.every(ac => isEvmChain !== isEthereumAddress(ac.address))) {
      return false;
    }
...
const ClaimReward = ({
  ...
  const reward = useMemo((): EarningRewardItem | undefined => {
    return earningRewards.find(item => item.slug === slug && item.address === reformatAddress(fromValue));
