...
function useLedgerBluetooth(deviceId?: string): UseLedgerBluetoothHook {
  ...
  const [isAppLaunchConfirmationNeeded, setIsAppLaunchConfirmationNeeded] = useState<boolean>(false);
  ...
  const setUpBluetoothConnection = async () => {
    ...
    if (!transportRef.current && deviceId) {
      try {
        const BluetoothTransport: any = await import('@ledgerhq/react-native-hw-transport-ble');
    ...
    try {
      ...
      if (appName === 'BOLOS') {
        ...
        try {
          ...
          console.log('open ethereum app on ledger');
        } catch (e: any) {
            ...
            switch (e.statusCode) {
              ...
              case 0x6807:
                throw new LedgerError(
                  'ledger.ethereum_app_not_installed_error',
                  LedgerCommunicationErrors.AppIsNotInstalled,
                );
              case 0x6985:
              case 0x5501:
                throw new LedgerError(
                  'ledger.ethereum_app_unconfirmed_error',
                  LedgerCommunicationErrors.UserRefusedConfirmation,
                );
            }
          }

          throw new LedgerError('ledger.ethereum_app_open_error', LedgerCommunicationErrors.FailedToOpenApp);
        } finally {
        ...
        try {
          console.log('close running app on ledger');
          // await closeRunningAppOnLedger();
        } catch (e) {
          throw new LedgerError('ledger.running_app_close_error', LedgerCommunicationErrors.FailedToCloseApp);
        }
      ...
      } else if (e.message.includes('Only version 4 of typed data signing is supported')) {
  ...
  return {
    ...
    ledgerLogicToRun: async func => {
      ...
      workflowSteps.current.push(() => func(transportRef.current as BluetoothInterface));
      //  Start off workflow
