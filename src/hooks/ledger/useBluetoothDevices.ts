...
const useBluetoothDevices = (hasBluetoothPermissions: boolean, bluetoothOn: boolean) => {
  ...
  useEffect(() => {
    ...
    if (hasBluetoothPermissions && bluetoothOn) {
      import('@ledgerhq/react-native-hw-transport-ble').then((bluetoothInterface: any) => {
        subscription = new Observable(bluetoothInterface.default.listen).subscribe({
          next: (e: any) => {
  ...
  }, [bluetoothOn, hasBluetoothPermissions]);
