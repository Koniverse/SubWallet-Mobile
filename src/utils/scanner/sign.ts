...
export const unwrapMessage = (message: string) => {
  ...
  unwrapped =
    unwrapped.substr(0, prefix.length) === prefix ? unwrapped.substr(prefix.length, unwrapped.length) : unwrapped;
  unwrapped =
    unwrapped.substr(unwrapped.length - suffix.length, suffix.length) === suffix
      ? unwrapped.substr(0, unwrapped.length - suffix.length)
      : unwrapped;
...
export function isSubstrateCompletedParsedData(
  ...
  return (parsedData as SubstrateCompletedParsedData)?.data?.crypto !== undefined;
}
...
export function isMultipartData(parsedData: ParsedData | null): parsedData is SubstrateMultiParsedData {
  ...
  return (parsedData as SubstrateMultiParsedData)?.isMultipart || hasMultiFrames;
}
