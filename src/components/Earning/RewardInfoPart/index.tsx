...
const RewardInfoPart: React.FC<Props> = (props: Props) => {
  const { inputAsset, compound, isShowBalance, rewardHistories, transactionChainValue, transactionFromValue } = props;
  ...
  const onPressWithdraw = useCallback(() => {
    ...
    if (total && new BigN(total).gt(BN_ZERO)) {
      navigation.navigate('Drawer', {
        ...
        params: {
          screen: 'ClaimReward',
          params: { slug: slug, chain: transactionChainValue, from: transactionFromValue },
        },
      });
    } else {
