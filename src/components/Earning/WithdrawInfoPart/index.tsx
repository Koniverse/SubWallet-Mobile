...
const WithdrawInfoPart: React.FC<Props> = (props: Props) => {
  const { unstakings, inputAsset, poolInfo, transactionChainValue, transactionFromValue } = props;
  const { slug } = poolInfo;
  ...
  const onPressWithdraw = useCallback(() => {
    navigation.navigate('Drawer', {
      ...
      params: { screen: 'Withdraw', params: { slug, chain: transactionChainValue, from: transactionFromValue } },
    });
  }, [navigation, slug, transactionChainValue, transactionFromValue]);

  const onPressCancelWithdraw = useCallback(() => {
    navigation.navigate('Drawer', {
      screen: 'TransactionAction',
      params: { screen: 'CancelUnstake', params: { slug, chain: transactionChainValue, from: transactionFromValue } },
    });
  }, [navigation, slug, transactionChainValue, transactionFromValue]);
...
export default WithdrawInfoPart;
