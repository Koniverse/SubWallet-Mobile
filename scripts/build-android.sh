...
java -jar /Volumes/MacData/App/bundletool.jar build-apks --bundle=$PWD/app/build/outputs/bundle/release/app-release.aab --output=$PWD/app/build/outputs/apk/build.apks --mode=universal --ks=$PWD/app/my-upload-key.keystore --ks-pass=pass:$KS_PASSWORD --ks-key-alias=my-key-alias --key-pass=pass:$KS_PASSWORD
